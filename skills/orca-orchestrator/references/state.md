...
Operator/environment maintenance (see [guardrails.md](guardrails.md#operatorenvironment-maintenance)) deliberately has no counter of its own here: a maintenance-blocked worker start still increments `dispatches` like any other dispatch, so it is already indirectly bounded by `max_dispatches` without a dedicated field. What it does not get is a `kind: execution` observation — it produced no execution to describe. It is instead recorded, when it matters, as an Owner gate or as a note on the *retry* dispatch's `kind: execution` observation once work resumes (see [Observation schema](#observation-schema)).
...
- `kind: "execution"` — one worker's dispatch in one role (Developer, Tester, or Reviewer) that reached task-related execution. Carries a well-defined `harness`/`model`/`backend`, because it describes what actually ran. A task with separate Developer/Tester/Reviewer dispatches (see [workflow.md](workflow.md#roles)) produces one execution observation per such dispatch, not one per task — except a dispatch blocked by [operator/environment maintenance](guardrails.md#operatorenvironment-maintenance) before it could attempt the task, which still counts toward `max_dispatches` but gets no execution observation, since it describes no actual execution.
